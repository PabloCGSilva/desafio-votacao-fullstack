...
import { createVotingSession } from '../../services/sessionService';
...
const AgendaList = () => {
  ...
  const fetchAgendas = async () => {
    try {
      ...
      setError('Failed to load agendas. Please try again later.');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateSession = async () => {
    try {
      ...
      setAlert({
        open: true,
        message: error.response?.data?.message || 'Failed to create voting session',
        severity: 'error'
      });
    }
  };
  ...
  return (
    <Box>
                ...
                <ListItem
                  ...
                  secondaryAction={
                    <Button
                      ...
                      onClick={() => handleCreateSessionClick(agenda.id, agenda.title)}
                    >
                      Open Voting
                    </Button>
                  }
      ...
      <Dialog open={sessionDialog.open} onClose={handleCloseDialog}>
        ...
        <DialogContent>
          <DialogContentText>
            You are opening a voting session for agenda: <strong>{sessionDialog.title}</strong>
          </DialogContentText>
          <TextField
            autoFocus
            margin="dense"
        ...
        <DialogActions>
          ...
          <Button onClick={handleCreateSession} variant="contained">Open Session</Button>
        </DialogActions>
      </Dialog>
